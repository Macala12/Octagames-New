...
ClassFruit.prototype.set = function(hide) {
    ...
    this.shadow = layer.createImage("fruit", "images/shadow.png", 
        (this.startX - radius) * scale.width, 
        (this.startY - radius + shadowPos) * scale.height, 
        106 * scale.width, 77 * scale.height);
    this.image = layer.createImage("fruit", inf[0], 
        (this.startX - radius) * scale.width, 
        (this.startY - radius) * scale.height, 
        inf[1] * scale.width, inf[2] * scale.height);
...
ClassFruit.prototype.pos = function(x, y) {
    ...
    this.shadow.attr({ 
        x: (x - r) * scale.width, 
        y: (y - r + shadowPos) * scale.height 
    });
    
    if (this.type === "boom") {
        this.flame.pos((x + 4) * scale.width, (y + 5) * scale.height);
    }
