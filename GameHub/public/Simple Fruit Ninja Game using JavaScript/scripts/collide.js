...
exports.check = function(knife) {
    ...
    // Scale knife coordinates
    var scaledKnife = [
        knife[0] * scale.width, knife[1] * scale.height,
        knife[2] * scale.width, knife[3] * scale.height
    ];
    
    fruits.forEach(function(fruit) {
        var ck = lineInEllipse(
            scaledKnife.slice(0, 2),
            scaledKnife.slice(2, 4),
            [fruit.originX * scale.width, fruit.originY * scale.height],
            fruit.radius * scale.width
        );
